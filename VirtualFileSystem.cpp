...
{ 
	...
	int FileActualSize; 
	int FileType; 
	char *Buffer;	//char* Buffer[5]; for increasing the file size 
	int LinkCount; 
	int ReferenceCount; 
	int permission; 
	struct inode *next;  
...
PINODE head = NULL; // Pointer to the linked list of inodes
 
void man(char *name) // Displays the description and usage of each command
{
	if(name == NULL) return; 
...
PINODE Get_Inode(char *name)	//to get the inode by name
{
	PINODE temp = head;
	int i = 0;
	...
	while(temp != NULL)	// Traversal till the name is found or end of the linked list
	{
		if(strcmp(name,temp->FileName) == 0) 
        	{
			break;		// stop loop if the name is found
        	}
		temp = temp->next;
	}
	
	return temp;	// temp is NULL if end is reached and if name not found...
}
...
{ 
	...
	while(i <= MAXINODE) // Insertlast in loop
	{ 
		...
		if(temp == NULL) // ie linked list empty
		{
			head = newn;
			temp = head;
		}
...
{ 
	...
	//to prevent garbage in ufdt array
	while(i < 50)
	{ 
...
{ 
	...
	if((name == NULL) || (permission == 0) || (permission > 3)) // invalid parameters
    	{
		return -1;
    	}
	if(SUPERBLOCKobj.FreeInode == 0)	// Linkedlist of inodes is full...(no free inode available)
    	{
		return -2;
    	}
	if(Get_Inode(name) != NULL)	// File already exists
    	{
		return -3;
    	}
	...
	while(temp != NULL)		// traversal till the free inode
	{
		if(temp -> FileType == 0)	//inode unused
		break;
		temp = temp -> next;
	}
	
	while(i < 50) // traversal till the free UFDT Array of structures
	{
		if(UFDTArr[i].ptrfiletable == NULL)
		break;
		i++;
	...
	// Memory allocation for filetable
	UFDTArr[i].ptrfiletable = (PFILETABLE)malloc(sizeof(FILETABLE));
	if(UFDTArr[i].ptrfiletable == NULL) //malloc failed
    	{
		return -4;
    	}
	
	// Filetable initialisation
	...
	// memory allocation for new file
	UFDTArr[i].ptrfiletable->ptrinode->Buffer = (char *)malloc(MAXFILESIZE);
	
	// to remove garbage from allocated memory
	memset(UFDTArr[i].ptrfiletable->ptrinode->Buffer,0,MAXFILESIZE);
	// used to replace byte by byte
	// parameter : address,set with value,size(upto which point)
...
int main() 
{
	...
	while(1) 
	{
		fflush(stdin); // to flush the keyboard
		strcpy(str,""); // to remove garbage from str
		
		printf("\nMarvellous VFS : > ");
		fgets(str,80,stdin);

		count = sscanf (str," %s %s %s %s",command[0],command[1],command[2],command[3]); // tokenise the str
		...
		{ 
			...
			{
				system("cls"); // to call the system call 'cls'
				continue;
			}
